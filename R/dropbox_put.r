...
#' }
dropbox_put <- function(cred, what, filename = what, curl = getCurlHandle(), 
    ..., verbose = FALSE) {
    ...
    size <- if (is(what, "AsIs")) {
        if (is.character(what)) 
            sum(nchar(what)) else length(what)
    } else file.info(what)[1, "size"]
    drop_save <- fromJSON(OAuthRequest(cred, url, , "PUT", upload = TRUE, 
        readfunction = input, infilesize = size, httpheader = c(`Content-Type` = "application/octet-stream"), 
        ..., curl = curl))

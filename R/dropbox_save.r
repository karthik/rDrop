...
#'}
dropbox_save <- function(cred,..., list = character(), file = stop("'file' must be specified"), envir = parent.frame(), precheck=TRUE,curl=getCurlHandle()) {
 ...
 filename <- paste(str_trim(str_extract(file, "[^.]*")), ".rda", sep = "")
 url <- paste("https://api-content.dropbox.com/1/files_put/dropbox/",filename, sep="")
...
serialize(list(a = 1:10, b = letters), con)
    ...
    xx <- OAuthRequest(cred, url,
        , "PUT", upload = TRUE, readfunction = input, infilesize = nchar(z),
        verbose = FALSE, httpheader = c(`Content-Type` = "application/binary"))
    close(con)
  fromJSON(xx)
}

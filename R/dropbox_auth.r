...
#'If you are unable to do so (assuming you are on some public machine), then just specifiy both keys inline. After your initial authentication is complete, there is absolutely no reason to repeat this step for subsequent sessions. Simply save the \code{DropboxCredentials} object and load as necessary. Future versions of \code{ROAuth} will make it easier for you to just update the token (when necessary) without having to reauthoize via the web. \emph{Caution: Do not store these keys in your .rprofile if you are on a public machine}. Anyone with access to the key + secret combo has full access to the contents of your Dropbox.
#'

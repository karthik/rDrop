...
{
	#Checking to make sure you specify keys one way or another
	if(is.null(cKey) && is.null(cSecret))
	{
		...
		if(length(cKey)==0 | length(cSecret)==0)
			{
				stop("Could not find your Dropbox keys in the function arguments or in your options. ?rdRrpbx for more help")
			}
	}
...
cred <- OAuthFactory$new(consumerKey = cKey, consumerSecret = cSecret,requestURL = reqURL,accessURL = accessURL,authURL = authURL)
...
return(cred)
 ...
 # mydropbox_tokens <- dropbox_auth("consumey_key","consumer_secret")